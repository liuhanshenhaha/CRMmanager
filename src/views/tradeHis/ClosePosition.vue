<template>
	<section>
		...
		<el-table class="restyle-table" show-summary :summary-method="getSummaries" stripe border fixed :data="tableData" highlight-current-row v-loading="listLoading" @selection-change="" style="width: 100%;">
			...
			<el-table-column prop="futureCode" label="合约代码" width="100">
			</el-table-column>
			...
			<el-table-column prop="kcjj" label="开仓价格" width="100">
			</el-table-column>
			<el-table-column prop="tradePrice" label="平仓价格" width="100">
			</el-table-column>
			<el-table-column prop="yk" label="平仓盈亏" width="100">
			</el-table-column>
			...
			<el-table-column prop="openPositionTime" label="开仓时间" width="100">
			</el-table-column>
			<el-table-column prop="tradeTime" label="平仓时间" width="150">
			</el-table-column>
...
<script>
	...
	import { tradeHisQuest } from "../../api/api"
	export default {
		data(){
			return {
				filters:{
		...
		methods:{
			...
			getList(pageNo){
				...
				if(this.customerType !== "customer"){
					tradeHisQuest.queryClosePosition(submitData).then(res => {
					...
					tradeHisQuest.queryUserClosePosition(submitData).then(res => {
						this.tableData = res.content.dataList;
						this.tableDataSummary = res.content.statisticsData;
						this.tableDataTotal = res.content.count;
						this.listLoading = false;
